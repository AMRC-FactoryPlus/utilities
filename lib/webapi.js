...
export class FplusHttpAuth {
    ...
    async auth (req, res, next) {
        ...
        try {
            ...
            if (!auth) throw "No HTTP auth provided";
...
export class WebAPI {
    ...
    async init () {
        ...
        /* Set caching */
        if (this.max_age) {
            /* must-revalidate is required for clients to cache
             * authenticated responses (don't ask me...) */
            const cc = `max-age=${this.max_age}, must-revalidate`
            app.use((req, res, next) => {
