...
export default class Fetch extends ServiceInterface {
    ...
    async do_fetch (url, opts) {
        ...
        const try_fetch = async () => {
            token = await this._service_token(opts.service_url, token);
    ...
    async _service_token (service, bad) {
        /* This might be a token or a promise for one */
        let token = this.tokens.get(service);
        ...
        if (!token || is_bad) {
            token = this._fetch_token(service);
        ...
        try {
            token = await token;
            this.debug.log("token", `Using token ${token} for ${service}`);
            return token;
        }
        catch (e) {
    ...
    async _fetch_token (service) {
        const tok_url = new URL("/token", service);
        ...
        if (!res?.ok) {
            ...
            throw `Token fetch fetch failed for ${service}`;
        }

        const json = await res.json();
        /* There will be a promise in the cache already; overwrite it
         * with the real token. */
        this.tokens.set(service, json.token);
        return json.token;
    }
