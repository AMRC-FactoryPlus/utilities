...
export default class ConfigDB extends ServiceInterface {
    ...
    async get_config (app, obj) {
        ...
        if (st == 404) return;
        if (st != 200)
            throw `ConfigDB: can't get ${app} for ${obj}: ${st}`;
        return json;
    }
    ...
    async put_config (app, obj, json) {
        ...
        if (st == 204) return;
        throw `ConfigDB: can't set ${app} for ${obj}: ${st}`;
    }
    ...
    async delete_config (app, obj) {
        ...
        if (st == 204) return;
        throw `ConfigDB: can't remove ${app} for ${obj}: ${st}`;
    }
    ...
    async create_object (klass, obj, excl) {
        ...
        if (st == 200 && excl)
            throw `ConfigDB: Exclusive create of ${obj} failed`;
        if (st == 201 || st == 200)
            return json.uuid;
        if (obj)
            throw `ConfigDB: Creating ${obj} failed: ${st}`;
        else
            throw `ConfigDB: Creating new ${klass} failed: ${st}`;
    }
    ...
    async delete_object (obj) {
        ...
        if (st == 204) return;
        throw `ConfigDB: Deleting ${obj} failed: ${st}`;
    }
